...
public class UserVerTool {
    ...
    /**
     * 校验生日，此处为字符串，允许各种格式输入，原样保存和显示
     * @param birthday
     * @param result
     */
    public static void verBirthday(String birthday, UserResult result) {
        //正则表达式
        boolean matches = Pattern.matches("^[A-Za-z0-9\\u2E80-\\u9FFF\\-]{0,15}$",birthday);
        if(!matches){
            result.setBirthdayError("限15个汉字、字母、数字和-");
        }
    }
